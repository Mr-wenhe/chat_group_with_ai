...
状态：S1–S7 实施完成；S8 自动化、真实 DOCX、原生窗口启动和一轮真实群聊讨论已检查，精确任务和原生聊天/授权 UI 闭环仍待现场验证
日期：2026-09-15
...
1. 私聊固定为当前角色，不运行群讨论、不推举群成员，也不读取其他会话的任务上下文。
2. 群聊每个工作任务先建立讨论状态；`@all` 只召集讨论成员。用户写明最终执行人时，路由器把该角色固定为最终责任人和执行人，不能被建议或旧阶段顺序覆盖。
3. 未指定执行人时，路由器只生成合格候选和讨论参与者；`WorkDiscussionRunner` 让群成员按自己的职业和模型公开发言，再由协调人记录推举依据。HTML/网页代码必须通过本地职业资格检查，不能靠名字、标签或全局 Skill 冒充前端。
4. 讨论摘要每轮写入参与记录、采纳/不采纳取舍、未决问题、阻碍和理解百分比。执行人资格、请求版本、DOCX/路径合同、100%理解度和空问题/阻碍共同组成唯一执行门禁。
5. 只有明确配置的旧式多阶段接力才产生后续阶段；普通群聊请求中的“需求、开发、测试”只是咨询与建议，不自动串成多个执行人。任一时刻只有最终执行人能取得工具执行权。
6. 路由器返回未知 ID、职业不匹配、模型不可用或群成员缺失时，本地拒绝工具执行并持久化 @用户 阻碍；不得回退到首位成员或让协调人代执行。

无需新增独立职业注册中心。角色配置中的职业、职责和已绑定能力是资格证据；全局技能和讨论文本只能作为辅助资料，不能改变执行人、产物合同或审批边界。
...
- 持久上下文、原文件修订、澄清后续跑。
- 角色路由、产品/开发/测试公开讨论和单一最终执行人。
- 结构化命令、流式输出、取消、轻量检查、缺失工具安装提示。

验收：追问修改同一路径；群角色先按职业讨论、由最终执行人收敛并独占执行权；命令流可见、停止可用；模型失败不自动切换；默认不跑完整测试/build。
...
S8 的集成测试必须从真实输入、`WorkRoleRouter`、`WorkDiscussionRunner`、`WorkTaskCoordinator` 和生产 `DefaultWorkTaskRunner` 进入 `WorkCommandRunner`，只替换模型传输为脱敏的结构化测试网关；测试网关不能替代真实模型质量结论。截图场景的最终合同是产品经理单一执行人、DOCX、用户指定桌面；HTML 只作为讨论主题，不得自动生成 HTML 代码。

讨论完成后才能投影工具批准；普通写入、转换命令、缺失转换工具、未授权路径和附件交付分别保留自己的等待状态。转换源 Markdown 是中间文件，必须由真实转换器生成并由结构、正文、路径和打开检查共同验证的 DOCX 才能完成。执行补充按 FIFO 进入同一任务；完成后的新交付重新讨论和选人。私聊沿用固定角色和私聊上下文，不参加群内推举。

讨论协议的非结构化回复只允许作为脱敏诊断和阻塞证据展示；即使诊断预览含有换行或其他不符合持久状态约束的字符，也不得写入 `decisionSummary`。只有通过 `WorkDiscussionTurn` 结构化校验的协调员回复才能更新该字段，状态写入仍必须经过 `WorkDiscussionState.isWithinBounds`。

S8 报告必须逐项列出 A01–A21、命令、真实环境、逐行审查、自动化证据、未检查项和剩余风险。精确工作任务的真实模型闭环、原生文件夹授权/撤销、转换器安装弹窗或 Microsoft Word GUI 缺失时，结论必须是“未检查/未完成验收”，不能用自动化绿灯替代现场证据；已完成的连接测试或普通群聊观察只能作为部分证据。
...
| 角色 | @all 召集、最终 @角色绑定、公开推举合格职业、单一执行人、私聊固定角色、角色 API 不可用、未知角色 ID |
...
9. 群聊请求“写需求、开发、测试”，观察成员按职业公开讨论、最终执行人汇总并独占执行权，不自动创建开发/测试接力。
...
- 无 P0/P1，所有 P2 都有明确结论；Windows 真实 UI、精确任务的原生输入闭环、原生授权/安装和 Word GUI 是当前明确暂缓项。
