...
- 第一阶段不要求所有关系都可视化编辑；数据模型预留初始关系能力，但 UI 可以后置。
...
- `affinity`：亲近度，建议范围 `-100..100`。
- `trust`：信任度，建议范围 `-100..100`。
- `friction`：摩擦度，建议范围 `0..100`。
- `familiarity`：熟悉度，建议范围 `0..100`。
- `recentMood`：最近情绪，例如 `warm`、`annoyed`、`awkward`、`protective`、`neutral`。
- `notes`：短文本关系备注。
- `lastInteractionAt`：最近互动时间。
- `createdAt`：创建时间。

第一阶段可通过本地规则自动创建关系记录；初始关系 UI 可以后置，但模型预留 notes 和初始分数即可。
...
- A 被 B 反驳：A 对 B 的 `friction` 上升，`recentMood` 可能变为 `annoyed`。
- 用户经常 @ 某角色：该角色对用户的 `familiarity` 上升，`affinity` 可小幅上升。
...
- 可在设置或群详情后续增加“真人感强度”开关，但第一阶段先使用默认强度 B。
- 初始关系编辑 UI 可后置；若本轮实现 UI，只做简单备注或关系类型，不做复杂关系图。

## 与现有代码的关系

- `AICharacter.memorySummary` 保留，作为迁移材料和兼容字段。
- `GroupMemory` 继续用于群体话题摘要，不替代角色级 `CharacterMemory`。
- `ChatOrchestrator` 可保留现有纯逻辑，也可逐步拆分到 `HumanizedChatOrchestrator`。
