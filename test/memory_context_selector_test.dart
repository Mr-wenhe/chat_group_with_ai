...
Future<void> main() async {
  ...
  group('User profile refresh', () {
    ...
    test('active memory conflicting with profile is excluded as a safeguard',
        () async {
      final db = DatabaseService();
      await db.userProfileBox.put(
          'me',
      ...
      expect(result, isNot(contains('用户叫小明')));
    });

    test('profile invalidation preserves audit data and records its reason',
        () async {
      final db = DatabaseService();
      ...
      await db.permanentMemoryBox.put(
        'pm:stable-profile-conflict',
        memory,
      );

      final count =
          await MemoryConflictResolver(db).invalidateConflictingProfileMemories(
        UserProfile(
          displayName: '小红',
          preferredAddress: '',
          avatar: '',
      ...
      final invalidated =
          db.permanentMemoryBox.get('pm:stable-profile-conflict')!;
      expect(count, 1);
      expect(invalidated.id, memory.id);
      expect(invalidated.status, MemoryStatus.invalidated);
      expect(invalidated.invalidationReason, 'profileOverride');
      expect(invalidated.originConversationId, 'group-1');
    });
  });
  ...
  group('Global relationship stable snapshot', () {
    ...
    test('fallback to latest legacy snapshot when no global snapshot',
        () async {
      final db = DatabaseService();
      await db.aiCharacterBox.put(
          'char-a',
  ...
  group('Selector invalidation chain', () {
    ...
    test(
        'deleting active memory removes it from prompt; deleting correction '
      ...
      await db.permanentMemoryBox
          .putAll({'prompt-parent': parent, 'prompt-correction': correction});
