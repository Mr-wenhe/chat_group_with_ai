...
/// Pure evidence collection and inference rules for legacy character genders.
class CharacterGenderInference {
  ...
  static List<Map<String, dynamic>> buildInferenceEntries(
    ...
    Iterable<Message> messages,
      ...
      buildInferenceEntriesFromReplies(
        characters,
        repliesByCharacter(characters, messages),
      );

  /// Builds the same bounded evidence from replies already read from storage.
  static List<Map<String, dynamic>> buildInferenceEntriesFromReplies(
    List<AICharacter> characters,
    Map<String, List<String>> replies,
  ) =>
      _buildInferenceEntries(characters, replies);
  ...
  /// Collects only recent AI replies belonging to the supplied characters.
  static Map<String, List<String>> repliesByCharacter(
    List<AICharacter> characters,
    Iterable<Message> messages,
  ) {
    ...
    for (final message in messages) {
      if (message.senderType == 'ai' && knownIds.contains(message.senderId)) {
        _retainLatestReply(replies[message.senderId]!, message);
      }
    }
    return {
  ...
  static void _retainLatestReply(List<Message> replies, Message message) {
    if (replies.length < maxRecentReplies) {
      replies.add(message);
      return;
    }
    var oldestIndex = 0;
    for (var index = 1; index < replies.length; index++) {
      if (_compareMessages(replies[index], replies[oldestIndex]) < 0) {
        oldestIndex = index;
      }
    }
    if (_compareMessages(message, replies[oldestIndex]) > 0) {
      replies[oldestIndex] = message;
    }
  }
